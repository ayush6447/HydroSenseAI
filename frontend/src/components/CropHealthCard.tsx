...
export default function CropHealthCard() {
  ...
  const startTest = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/predict/plant-health`, {
        method: 'POST',
        headers: {
      ...
      setTestResult({ diagnosis: data.diagnosis, confidence: data.confidence });
    } catch (e) {
      console.error("YOLO API Failed:", e);
      setTestResult({ diagnosis: "ERROR", confidence: "0%" });
    } finally {
      setIsTesting(false);
    }
  };
